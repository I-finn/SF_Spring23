...
#Similarly to machine learning, time series forecasting involves partitioning
#data to test the accuracy of models. However, whereas machine learning on 
#cross-sectional data involves partitioning into randomly assigned *sets*, 
#time series forecasting requires partitioning the data into *periods*, where
#the earlier period is the *training* period, and the later period the 
#*validation* period. (Note: there is no equivalent in time series forecasting
#* to the hold-out test set in ML.) Once the model has been trained and a 
#* final model selected, the training and validation sets are recombined, 
#* and the chosen model rerun on the complete data. 
...
full.lm <- tslm(amtrak.ts ~ trend + I(trend^2))
full.lm.pred <- forecast(full.lm, h = nValid, #we predict forward 36 periods
                         level = 0) #confidence level for prediction intervals
#Make a plot of the model and prediction
plot(full.lm.pred, ylim = c(1300, 2600), ylab = "Ridership", xlab = "Time", bty = "l",
     xaxt = "n", xlim = c(1991, 2009.25), main = "", flty = 2)
axis(1, at = seq(1991, 2009, 1), labels = format(seq(1991, 2009, 1))) # at tick marks for each year
lines(full.lm$fitted, lwd = 2) #add the model line in the training period
lines(amtrak.ts) #add the lines for the validation period
