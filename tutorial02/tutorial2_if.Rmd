...
axis(1, at = seq(1991, 2006, 1), labels = format(seq(1991, 2006, 1))) # at tick marks for each year
...
negative value for ME means over prediction - ie too high
...
# residuals look seasonal, so model is not accounting for seasonality
# want average residual to be 0

hist(c(train.lm$residuals, #histogram combining train and validation errors
       valid.ts - train.lm.pred$mean),
     xlab = "residual", main = "")
# looking for skew - over/under estimation
...
svs.ts <- ts(svs$Sales, start = c(sStarty, sStartm), end = c(sEndy,sEndm), frequency = 12) #convert to ts
plot(svs.ts)
axis(1, at = seq(sStarty, sEndy, 1), labels = format(seq(sStarty, sEndy, 1)))
```
```{r train svs}
#Partition into train and validation periods
length(svs.ts)
nValid <- 12*1 #The length of our validation period in months
nTrain <- length(svs.ts) - nValid #The length of our training period

#Partition the data into training and validation periods
train.ts <- window(svs.ts, start = c(sStarty, 1),
                   end = c(sStarty, nTrain)) #we use nTrain to set the end of the window
valid.ts <- window(svs.ts, start = c(sStarty, nTrain + 1),
                   end = c(sStarty, nTrain + nValid))
...
#Make a plot of the model and prediction
plot(train.lm.pred, ylim = c(1500, 105000), ylab = "Sales", xlab = "Time", bty = "l",
     xaxt = "n", xlim = c(sStarty, sEndy+.25), main = "", flty = 2)
axis(1, at = seq(sStarty, sEndy, 1), labels = format(seq(sStarty, sEndy, 1))) # at tick marks for each year
lines(train.lm$fitted, lwd = 2) #add the model line in the training period
lines(valid.ts) #add the lines for the validation period
```
```{r naive svs}
...
```{r plotting forecast errors svs}
fivenum(train.lm$residuals)

plot(train.lm$residuals, xlim = c(sStarty, sEndy), ylim = c(-9000, 30000), ylab = "residual")
lines(valid.ts - train.lm.pred$mean)
abline(v = sStarty+(nTrain/12)+0.25, col = "blue")
# residuals look seasonal, so model is not accounting for seasonality
# want average residual to be 0

plot(naive.pred$mean, xlim = c(sStarty, sEndy), ylim = c(-9000, 30000), ylab = "residual")
lines(valid.ts - naive.pred$mean)
abline(v = sStarty+(nTrain/12)+0.25, col = "blue")
#Histogram of errors
hist(c(train.lm$residuals, #histogram combining train and validation errors
       valid.ts - train.lm.pred$mean), xlim=c(2001, 2002),
     xlab = "residual", main = "LM" )

hist(c(naive.pred$residuals, #histogram combining train and validation errors
       valid.ts - naive.pred$mean), xlim=c(2001, 2002),
     xlab = "residual", main = "Naive")

hist(c(snaive.pred$residuals, #histogram combining train and validation errors
       valid.ts - snaive.pred$mean), xlim=c(2001, 2002),
     xlab = "residual", main = "Seasonal Naive")
...
plot(density(c(naive.pred$residuals, #a pdf
               valid.ts - naive.pred$mean)), xlim=c(2001, 2002),
     xlab = "residual", main = "")
...
plot(svs.ts, ylab = "Sales", xlab = "Time")
lines(naive.pred$mean, lty = 3, lwd = 2, col = "red")
lines(snaive.pred$mean, lty = 3, lwd = 2, col = "blue")
...
#plot(train.AAN.pred, ylab = "Sales")
