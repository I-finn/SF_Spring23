...
# centres - uses +- 6 months - can't use for forecasting
# shows average with seasonality removed
...
# clearer topleft/bottom right - seasonality
# clearer topright/bottom left - anti-seasonality
##  6 month values are anti correlated 
...
# bars 95% of the time.
#> inside the lines is noise - outside = sig correlation
ggAcf(amtrak.ts)
ggAcf(diff(diff(amtrak.ts,lag=1), lag=12))
...
#> window (lag) - matches seasonality
#> have to stationary the data first - otherwise won't work TODO
...
#> pp90-98
#> p-value - hyp test: no residual pattern - so higher better
#> df = no lags = no observations after applied lag
checkresiduals(ets.fc)

#	Ljung-Box test

#data:  Residuals from ETS(M,N,A)
#Q* = 20.578, df = 24, p-value = 0.6635

#Model df: 0.   Total lags used: 24

#> MNA - multiplicative error & no trend - counterintuitive
#> check AAA
