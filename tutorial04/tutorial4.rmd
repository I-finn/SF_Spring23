...
mtitle <- "Time Series:  Souvenier Sales (AUD)"
stitle <- paste0(startYear, " - ", endYear)
y_lab <- "Sales (AUD)"
x_lab <- "Year"
y_lim <- c(1600, 105000)              # range of val
x_lim <- c(startYear, endYear+11/12)# range of time

```
```{r souvenirVisual, eval = TRUE}
svs.ts %>% autoplot()
# seasonality + trend
svs.ts %>% ggtsdisplay()
# ac at 1, 12, 24
...
lambda <- BoxCox.lambda(svs.train)
lambda
...
# change title to match season and trend windows TODO
svs.stl %>% autoplot() +
  ggtitle(mtitle, subtitle = stitle) +
  xlab(x_lab) +
  ylab(y_lab)
svs.resid <- svs.stl$time.series[,3]
svs.resid %>% autoplot()

```
```{r ets, eval=FALSE}

svs.ets <- ets(svs.train, model="ANN")
svs.ets.pred <- forecast(svs.ets, h = nValid, level = 0)

plot(svs.ets.pred, ylim = y_lim, ylab = y_lab,
     xlab = x_lab, bty = "l", xaxt = "n", xlim = x_lim,
     main = "", flty = 2)
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted",
     lwd = par("lwd"), equilogs = TRUE)
axis(1, at = seq(startYear,endYear, 1), labels = format(seq(startYear,endYear,1)) )
lines(svs.ets.pred$fitted, lwd = 2, col = "blue")
lines(svs.valid)

accuracy(svs.ets.pred, svs.valid)
...
svs.ts %>% diff(lag=12) %>% ggtsdisplay()
svs.ts %>% diff(lag=1) %>% ggtsdisplay()

svs.ts %>% diff(lag=12)%>% diff(lag=12)%>% ggAcf()
svs.manual <- svs.train %>% diff(lag=12)%>% diff(lag=12)
checkresiduals(svs.manual)
svs.manual.forecast <-forecast(svs.manual, h=nValid)

accuracy(svs.manual.forecast, svs.valid)
...
svs.arima <- svs.train %>% auto.arima()
svs.arima.pred <- svs.arima %>% forecast()

plot(svs.arima.pred, ylim = y_lim, ylab = y_lab,
     xlab = x_lab, bty = "l", xaxt = "n", xlim = x_lim,
     main = "", flty = 2)
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted",
     lwd = par("lwd"), equilogs = TRUE)
axis(1, at = seq(startYear,endYear, 1), labels = format(seq(startYear,endYear,1)) )
lines(svs.arima.pred$fitted, lwd = 2, col = "blue")
lines(svs.valid)

accuracy(svs.arima.pred, svs.valid)
